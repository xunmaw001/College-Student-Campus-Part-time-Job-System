	...
	<script>
		...
		function setDataRow(item,number){
			...
			var gangweizhizeCell  = document.createElement('td');
						  ...
						  //如果图文详情包含图片
			  if(item.gangweizhize && item.gangweizhize.indexOf('img') != -1){
				//暂时只考虑图片+文字 和 文字+图片的情况
				var beginIndex = item.gangweizhize.indexOf('<img');
				var endIndex = item.gangweizhize.indexOf('>');				
				gangweizhizeCell.innerHTML = item.gangweizhize.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
			  }else{
			...
			crossBtn1.setAttribute("onclick","crossTable(" + item.id + ",\"qiuzhixinxi\")");
